...
static ssize_t btcoex_write(struct file *file_p, const char __user *buf_p, size_t count,
                            loff_t *pos_p)
{
    u8 *msg_buff = NULL;
...
static int __init btcoex_init(void)
{
    ...
    RTKBT_INFO("Realtek Bluetooth coex driver module init, version %s", RTKBT_RELEASE_NAME);
    err = btcoex_chr_init();
    if (err < 0)
    {
